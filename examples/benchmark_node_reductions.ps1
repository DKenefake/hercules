param(
    [string]$OutputDirectory = 'target/presolve-benchmark/node-reductions',
    [int]$Seconds = 30,
    [int]$HardSeconds = 45
)
...
$modes = @(
    @{name='baseline'; nodes='false'; blocks='false'},
    @{name='nodes'; nodes='true'; blocks='false'},
    @{name='nodes-blocks'; nodes='true'; blocks='true'}
)
foreach ($instance in @('mk487a', 'mk487b', 'mk487c')) {
    foreach ($mode in $modes) {
        ...
        docker exec -e OPENBLAS_NUM_THREADS=1 -e MKL_NUM_THREADS=1 -w /work hercules-dev `
            ...
            $mode.nodes $mode.blocks 2>&1 | Tee-Object -FilePath $log | ForEach-Object {
                if ($_ -match '^(ROOT_REDUCTION_CONFIG |REDUCTION |NODE_REDUCTION |PROBING |END_TO_END |VALIDATION )') { Write-Output $_ }
            }
        $code = $LASTEXITCODE
        ...
        foreach ($pair in [regex]::Matches($end, '(\w+)=(\S+)')) {
            $row[$pair.Groups[1].Value] = $pair.Groups[2].Value
        }
        $row.probing = $lines | Where-Object { $_ -match '^PROBING ' } | Select-Object -Last 1
        $row.reduction = $lines | Where-Object { $_ -match '^REDUCTION ' } | Select-Object -Last 1
        $row.node_reduction = $lines | Where-Object { $_ -match '^NODE_REDUCTION ' } | Select-Object -Last 1
