"""Smoke-test fresh Python bindings: python3 examples/check_node_probing_python.py /target/debug/libhercules.so"""
...
for size, expected_values in objectives.items():
    for index, expected in enumerate(expected_values, 1):
        ...
        for probes, weak_roof, relations, pairs, root_probes, symmetry in product(
            ...
            solution, objective, seconds, visited, processed = hercules.solve_branch_bound(
                problem,
                timeout=10.0,
            ...
            assert len(solution) == problem[-1] and all(value in (0, 1) for value in solution)
            assert abs(objective - expected) < 1e-5, (
                size, index, probes, weak_roof, relations, pairs, root_probes, symmetry, objective
            )
            checked += 1
            print(
...
for option in ("node_probe_max_seconds", "root_roof_probe_max_seconds"):
    for invalid in (-1.0, float("inf"), float("nan")):
        try:
            ...
            raise AssertionError(f"accepted invalid {option}={invalid}")
