...
impl Qubo {
    ...
    /// Convert the QUBO struct to the list format. This is useful for interoperability with Python.
    /// Returns (i, j, q, c, num_x) where i and j are the row and column indices of the non-zero elements of the Q matrix,
    /// q is the values of the non-zero elements of the Q matrix, c is the linear coefficients, and num_x is the number of variables.
    ///
    /// Example to convert a QUBO to the list format:
    /// ```rust
    /// use hercules::qubo::Qubo;
    /// use sprs::CsMat;
    /// use ndarray::Array1;
    ...
    /// let grad = p.eval_grad_usize(&x_0);
    /// ```
    ...
    /// use hercules::qubo::Qubo;
    ///
    /// let p = Qubo::read_qubo("test_data/test.qubo");
    /// ```
    ///
    /// # Panics
    ///
    ...
    /// Shifts the diagonal of the QUBO matrix by a factor of 's', this is a safe exact operation
    /// that generates an equivalent QUBO. For any x in {0,1}^n the following holds:
    ///
    /// $$\frac{1}{2}x^TQx + c^Tx = \frac{1}{2}x^T(Q + sI)x + c^Tx - 0.5 sx$$
    ///
    /// Example of shifting the diagonal of a QUBO:
    /// ```rust
    /// use hercules::qubo::Qubo;
    /// use ndarray::Array1;
    /// use smolprng::{PRNG, JsfLarge};
    ...
    /// let p = Qubo::make_random_qubo(50, &mut prng, 0.1);
    ///
    /// // shift the diagonal of the QUBO by 1.0
    /// let p_sym = p.make_diag_transform(1.0f64);
    /// ```
    #[must_use]
    ...
    /// Shifts the diagonal of the QUBO matrix by an array 's', this is a safe exact operation
    /// that generates an equivalent QUBO. For any x in {0,1}^n the following holds:
    ///
    ...
    /// let p = Qubo::make_random_qubo(50, &mut prng, 0.1);
    ///
    /// // shift the diagonal of the QUBO by a random array
    /// let s = (0..p.num_x()).map(|_| prng.gen_f64()).collect::<Array1<f64>>();
    /// let p_sym = p.make_diag_array_transform(&s);
    /// ```
    #[must_use]
    ...
    /// Creates the convex symmetric form of the QUBO problem using the standard spectral shift path.
    /// This is an exact operation, and results in a convex symmetric matrix that is equivalent to the original QUBO.
    #[must_use]
    pub fn convex_symmetric_form_standard(&self) -> Self {
        ...
        p_sym.make_diag_transform(min_eig.abs() * 1.1)
    }

    /// Creates the convex symmetric form of the QUBO problem using the SDP-initialized path.
    /// This is an exact operation, and results in a convex symmetric matrix that is equivalent to the original QUBO.
    #[must_use]
    pub fn convex_symmetric_form_sdp(&self) -> Self {
        let p_sym = self.make_symmetric();

        let rank = (2 * p_sym.num_x()).isqrt() + 1;
        ...
        let p_shifted = p_sym.make_diag_array_transform(&diag_shift);
        let eigs = p_shifted.hess_eigenvalues();
        let min_eig = eigs.iter().fold(f64::INFINITY, |acc, &x| x.min(acc));
        ...
        p_shifted.make_diag_transform(0.01 - min_eig)
    }
    ...
    /// The convex symmetric form is generated by first making the QUBO symmetric, then shifting the diagonal to make it convex.
    /// The default path is initialized using the mixing cut SDP solver.
    ///
    /// Example of creating the convex symmetric form of a QUBO:
    /// ```rust
    /// use hercules::qubo::Qubo;
    /// use smolprng::{PRNG, JsfLarge};
    ///
    ...
    /// let p = Qubo::make_random_qubo(50, &mut prng, 0.1);
    ///
    /// // make a convex symmetric QUBO from this QUBO
    /// let p_cs = p.convex_symmetric_form();
    /// ```
    #[must_use]
    pub fn convex_symmetric_form(&self) -> Self {
        self.convex_symmetric_form_sdp()
    }
...
mod tests {
    ...
    fn temp_qubo_path(name: &str) -> PathBuf {
        std::env::temp_dir().join(format!("hercules-{}-{name}", std::process::id()))
    }
    ...
    fn read_write_consistency() {
        ...
        let path = temp_qubo_path("test.qubo");
        let path_str = path.to_string_lossy().into_owned();
        Qubo::write_qubo(&p, &path_str);

        // now read it back in
        let q = Qubo::read_qubo(&path_str);
    ...
    fn large_scale_write_qubo() {
        ...
        let path = temp_qubo_path("test_large.qubo");
        let path_str = path.to_string_lossy().into_owned();
        Qubo::write_qubo(&p, &path_str);
